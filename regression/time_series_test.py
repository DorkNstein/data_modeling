...
import os
try:
    os.chdir(os.path.join(os.getcwd(), 'data_modeling/regression'))
    print(os.getcwd())
except:
    pass
# %% [markdown]
...
# %%
google = pd.read_csv('../input/Regression/stock-time-series-20050101-to-20171231/GOOGL_2006-01-01_to_2018-01-01.csv',
                     index_col='Date', parse_dates=['Date'])
google.head()


# %%
humidity = pd.read_csv('../input/Regression/historical-hourly-weather-data/humidity.csv',
...
google.High.pct_change().mul(100).plot(
    figsize=(20, 6))  # Another way to calculate returns
...
decomposed_google_volume = sm.tsa.seasonal_decompose(
    google["High"], freq=360)  # The frequncy is annual
...
rmse = math.sqrt(mean_squared_error(
    sim1[900:1011], result.predict(start=900, end=999)))
...
# %%
rmse = math.sqrt(mean_squared_error(humidity["Montreal"].diff(
).iloc[900:1000].values, result.predict(start=900, end=999)))
print("The root mean squared error is {}.".format(rmse))
...
# %%
rmse = math.sqrt(mean_squared_error(humidity["Montreal"].diff(
).iloc[1000:1101].values, result.predict(start=1000, end=1100)))
print("The root mean squared error is {}.".format(rmse))
...
# %%
rmse = math.sqrt(mean_squared_error(microsoft["Volume"].diff(
).iloc[1000:1101].values, result.predict(start=1000, end=1100)))
print("The root mean squared error is {}.".format(rmse))
...
# %%
rmse = math.sqrt(mean_squared_error(microsoft["Volume"].diff(
).iloc[700:1001].values, result.predict(start=700, end=1000)))
print("The root mean squared error is {}.".format(rmse))
...
train_sample = pd.concat(
    [google["Close"].diff().iloc[1:], microsoft["Close"].diff().iloc[1:]], axis=1)
...
# calculating error
rmse = math.sqrt(mean_squared_error(
    train_sample.iloc[1:1002].values, predicted_result.values))
print("The root mean squared error is {}.".format(rmse))
